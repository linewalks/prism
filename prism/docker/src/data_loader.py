...
class DataLoader:
  ...
  def groupby_hour_measurement(self, measurement_df):
    ...
    measurement_df = measurement_df.groupby(group_cols) \
        .VALUE_AS_NUMBER.agg(agg_list).fillna(0)

    measurement_df = measurement_df.unstack().groupby(['PERSON_ID']).fillna(method="ffill") \
                     .fillna(method="bfill").reset_index()

    if measurement_df.isnull().sum().sum() !=0:
        print("there is no value even after interpolation")
