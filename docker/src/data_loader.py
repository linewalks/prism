...
class DataLoader:
  def __init__(self, data_path='/data/train', is_train=True,
    ...
    self.task_path = os.path.join(self.data_path, '..', 'volume', task_id)
  ...
  def extract_condition(self):
    ...
    if self.is_train:
      self.condition_cols = condition_df.CONDITION_SOURCE_VALUE.unique()
      np.save(os.path.join(self.task_path, 'condition_list.npy'), self.condition_cols)
  ...
  def groupby_hour_condition(self, condition_df):
    ...
    if not self.is_train:
      # train에 나타났던 질병 코드만. train에 나왔던 질병코드가 없다면 0 값 채워서 추가
      condition_only_in_train_cols = {x: 0 for x in self.condition_cols if x not in condition_df.columns}
      condition_df = condition_df.assign(**condition_only_in_train_cols)

    print("data_loader groupby_hour_condition time:", time.time() - start_time)
    return condition_df.reset_index()
  ...
  def make_data(self):
    ...
    person_condition_map = self.condition_df.groupby('PERSON_ID').agg('sum')[self.condition_cols].to_dict('split')
    condition_val = {p: c for p, c in zip(person_condition_map['index'], person_condition_map['data'])}
    ...
    for row in cohort_df[cols].values:
      ...
      while True:
        ...
        if person_id == subject_id and condition_date < cohort_end_date:
          x_list_cond.append(condition_val[person_id])
          break
        elif person_id > subject_id:
          # 데이터를 못찾음. 다음 환자로 넘어가버렸다
          condition_val = np.array([0] * len(self.condition_cols))
          x_list_cond.append(condition_val)
          break
        else:
          # 진단 누계에서 가장 최근 진단정보 제거
          condition_val[person_id] = condition_val[person_id] - condition_row[2:]
          # 탐색이 더 필요함
          condition_idx += 1
